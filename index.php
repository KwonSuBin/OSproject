...
<form action="HRRN.php" method="post">
...
$a = 0; //다음에 올 프로세스
$empty = 1; //현재 실행되고 있는 프로세스가 있는지 검사(1이면 동작하고 있는 프로세스가 없음.)
$Response = array(); //응답률
...
//응답률 계산
  for ($i=0; $i <$p_num; $i++) {
    $Response[$i]= ($WT[$i]+$p_btime[$i])/$p_btime[$i];
  }

  //실행중인 프로세스가 없을 경우
  if($empty == 1){

    //대기큐에 대기 중인 프로세스가 없으면 종료
    ...
    //대기큐에 대기 중인 프로세스가 여러개일 경우
      ...
      //응답률을 비교할 대상(max)를 설정
      for($i = 0; $i < $p_num; $i++){
        if($p_btime[$waiting_Queue[$i]] > 0)
          $max = $waiting_Queue[$i];
      }

      //대기 중인 프로세스 중 응답률을 비교해 더 큰 것을 찾는다.
      //응답률이 큰 것을 다시 max로 설정해 다음 것과 계속해서 비교한다.
      for($i=1;$i <$p_num; $i++){
        if($p_btime[$waiting_Queue[$i]] > 0 && $Response[$max] < $Response[$waiting_Queue[$i]]){
          $max = $waiting_Queue[$i];
        }
      }
      //응답률이 가장 큰 것을 실행 시킨다.
    ...
    //실행 중인 프로세스가 있다는 것을 표시
    $empty = 0;
  }

  //실행 중인 프로세스가 있으면 실행중인 프로세스의 burst time을 차감
  if($empty == 0){
    $p_btime[$running]--;
  }
...
//어떤 프로세스가 어떤 색상을 그래프에서 사용하는지 명시
if($p_num >= 1)
